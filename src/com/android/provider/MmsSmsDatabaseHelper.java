...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static String getDatabasePath(Context context) {
        String dbPath = MmsConfig.DATA_PATH;
        if (!Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            Toast.makeText(context, "ERROR: No sdcard!", Toast.LENGTH_LONG).show();
            return null;
        }
        ...
        return dbPath + File.separator + DATABASE_NAME;
    }
        ...
        try {
            ...
            if (rows > 0) {
                ...
                String query = "SELECT thread_id FROM sms WHERE type=" +
                        IMessage.TextBasedSmsColumns.MESSAGE_TYPE_FAILED +
            ...
            try {
                if (smsRows != null) {
                    ...
                    while (smsRows.moveToNext()) {
                        ...
                        cv.put(IMessage.MmsSms.WordsTable.ID, id);
                        cv.put(IMessage.MmsSms.WordsTable.INDEXED_TEXT, body);
                        cv.put(IMessage.MmsSms.WordsTable.SOURCE_ROW_ID, id);
