...
import com.android.provider.IMessage.CanonicalAddressesColumns;
...
import com.android.mms.util.DownloadDrmHelper;
...
public class MmsProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (table.equals(TABLE_PDU)) {
            ...
            if (!plainText && !smilText) {
                ...
                // permission settings.
                String path = MmsConfig.DATA_PATH + "/PART_" + System.currentTimeMillis() + contentLocation;
            ...
            if (plainText) {
                ...
                cv.put(IMessage.MmsSms.WordsTable.ID, (2 << 32) + rowId);
                cv.put(IMessage.MmsSms.WordsTable.INDEXED_TEXT, values.getAsString("text"));
                cv.put(IMessage.MmsSms.WordsTable.SOURCE_ROW_ID, rowId);
                cv.put(IMessage.MmsSms.WordsTable.TABLE_ID, 2);
            ...
            String path = MmsConfig.DATA_PATH + "/PART_" + System.currentTimeMillis();
    ...
    public int update(Uri uri, ContentValues values,
        ...
        switch (match) {
            ...
            case MMS_PART_RESET_FILE_PERMISSION:
                String path = MmsConfig.DATA_PATH + '/' + uri.getPathSegments().get(1);
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
        ...
        try {
            ...
            if (!filePath.getCanonicalPath()
                    .startsWith(MmsConfig.DATA_PATH + File.separator)) {
                return null;
            }
        } catch (IOException e) {
            return null;
    ...
    static {
        sURLMatcher.addURI("mms-yang", null,         MMS_ALL);
        sURLMatcher.addURI("mms-yang", "#",          MMS_ALL_ID);
        sURLMatcher.addURI("mms-yang", "inbox",      MMS_INBOX);
        sURLMatcher.addURI("mms-yang", "inbox/#",    MMS_INBOX_ID);
        sURLMatcher.addURI("mms-yang", "sent",       MMS_SENT);
        sURLMatcher.addURI("mms-yang", "sent/#",     MMS_SENT_ID);
        sURLMatcher.addURI("mms-yang", "drafts",     MMS_DRAFTS);
        sURLMatcher.addURI("mms-yang", "drafts/#",   MMS_DRAFTS_ID);
        sURLMatcher.addURI("mms-yang", "outbox",     MMS_OUTBOX);
        sURLMatcher.addURI("mms-yang", "outbox/#",   MMS_OUTBOX_ID);
        sURLMatcher.addURI("mms-yang", "part",       MMS_ALL_PART);
        sURLMatcher.addURI("mms-yang", "#/part",     MMS_MSG_PART);
        sURLMatcher.addURI("mms-yang", "part/#",     MMS_PART_ID);
        sURLMatcher.addURI("mms-yang", "#/addr",     MMS_MSG_ADDR);
        sURLMatcher.addURI("mms-yang", "rate",       MMS_SENDING_RATE);
        sURLMatcher.addURI("mms-yang", "report-status/#",  MMS_REPORT_STATUS);
        sURLMatcher.addURI("mms-yang", "report-request/#", MMS_REPORT_REQUEST);
        sURLMatcher.addURI("mms-yang", "drm",        MMS_DRM_STORAGE);
        sURLMatcher.addURI("mms-yang", "drm/#",      MMS_DRM_STORAGE_ID);
        sURLMatcher.addURI("mms-yang", "threads",    MMS_THREADS);
        sURLMatcher.addURI("mms-yang", "resetFilePerm/*",    MMS_PART_RESET_FILE_PERMISSION);
    }
