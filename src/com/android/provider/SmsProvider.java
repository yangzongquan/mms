...
public class SmsProvider extends ContentProvider {
    private static final Uri NOTIFICATION_URI = Uri.parse("content://sms-yang");
    ...
    public Cursor query(Uri url, String[] projectionIn, String selection,
        ...
        // TODO: Since the URLs are a mess, always use content://sms-yang
    ...
    private Uri insertInner(Uri url, ContentValues initialValues) {
        ...
        if (table == TABLE_SMS) {
            ...
            cv.put(IMessage.MmsSms.WordsTable.ID, rowID);
            cv.put(IMessage.MmsSms.WordsTable.INDEXED_TEXT, values.getAsString("body"));
            cv.put(IMessage.MmsSms.WordsTable.SOURCE_ROW_ID, rowID);
            cv.put(IMessage.MmsSms.WordsTable.TABLE_ID, 1);
            db.insert(TABLE_WORDS, IMessage.MmsSms.WordsTable.INDEXED_TEXT, cv);
        }
        if (rowID > 0) {
            Uri uri;
            if (table.equals(TABLE_SMS)) {
                uri = Uri.parse("content://sms-yang/" + rowID);
            } else {
                uri = Uri.parse("content://" + table + "/" + rowID);
            }
    ...
    private void notifyChange(Uri uri) {
        ...
        cr.notifyChange(Uri.parse("content://mms-sms-yang/conversations/"), null);
    }
    ...
    static {
        sURLMatcher.addURI("sms-yang", null, SMS_ALL);
        sURLMatcher.addURI("sms-yang", "#", SMS_ALL_ID);
        sURLMatcher.addURI("sms-yang", "inbox", SMS_INBOX);
        sURLMatcher.addURI("sms-yang", "inbox/#", SMS_INBOX_ID);
        sURLMatcher.addURI("sms-yang", "sent", SMS_SENT);
        sURLMatcher.addURI("sms-yang", "sent/#", SMS_SENT_ID);
        sURLMatcher.addURI("sms-yang", "draft", SMS_DRAFT);
        sURLMatcher.addURI("sms-yang", "draft/#", SMS_DRAFT_ID);
        sURLMatcher.addURI("sms-yang", "outbox", SMS_OUTBOX);
        sURLMatcher.addURI("sms-yang", "outbox/#", SMS_OUTBOX_ID);
        sURLMatcher.addURI("sms-yang", "undelivered", SMS_UNDELIVERED);
        sURLMatcher.addURI("sms-yang", "failed", SMS_FAILED);
        sURLMatcher.addURI("sms-yang", "failed/#", SMS_FAILED_ID);
        sURLMatcher.addURI("sms-yang", "queued", SMS_QUEUED);
        sURLMatcher.addURI("sms-yang", "conversations", SMS_CONVERSATIONS);
        sURLMatcher.addURI("sms-yang", "conversations/*", SMS_CONVERSATIONS_ID);
        sURLMatcher.addURI("sms-yang", "raw", SMS_RAW_MESSAGE);
        sURLMatcher.addURI("sms-yang", "attachments", SMS_ATTACHMENT);
        sURLMatcher.addURI("sms-yang", "attachments/#", SMS_ATTACHMENT_ID);
        sURLMatcher.addURI("sms-yang", "threadID", SMS_NEW_THREAD_ID);
        sURLMatcher.addURI("sms-yang", "threadID/*", SMS_QUERY_THREAD_ID);
        sURLMatcher.addURI("sms-yang", "status/#", SMS_STATUS_ID);
        sURLMatcher.addURI("sms-yang", "sr_pending", SMS_STATUS_PENDING);
        sURLMatcher.addURI("sms-yang", "icc", SMS_ALL_ICC);
        sURLMatcher.addURI("sms-yang", "icc/#", SMS_ICC);
        //we keep these for not breaking old applications
        sURLMatcher.addURI("sms-yang", "sim", SMS_ALL_ICC);
        sURLMatcher.addURI("sms-yang", "sim/#", SMS_ICC);
